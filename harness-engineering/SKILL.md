...
description: >
  This skill should be used when the user wants to "set up harness engineering",
  "bootstrap a repo for agents", "add control-system primitives", "create AGENTS.md",
  "add observability docs", "set up agent workflows", or needs to implement OpenAI
  Harness Engineering practices in a repository for autonomous agent development.
version: 1.0.0
author: Mikko Kohtala
tags:
  [harness-engineering, agent-workflows, control-system, observability, openai]
---

# Harness Engineering Playbook

Operationalize the practices from OpenAI's Harness Engineering guide in a repo that agents can run against repeatedly and safely.
...
```bash
$SKILL_DIR/scripts/bootstrap_harness.sh <repo-path>
```

This installs safe defaults from `assets/templates/`:
...
- **`references/openai-harness-practices.md`** — Full practice-to-artifact mapping with verification criteria.
- **`references/rollout-checklist.md`** — Phased adoption checklist for existing repositories.
- **`references/wizard-cli.md`** — Typer wizard CLI command reference.

### Scripts

- **`scripts/harness_wizard.py`** — Typer CLI for init, audit, status, and primitive management.
- **`scripts/bootstrap_harness.sh`** — Shell bootstrap fallback for template installation.
- **`scripts/audit_harness.sh`** — Baseline harness completeness audit.
